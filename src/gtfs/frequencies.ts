...
function getMeanFrequency(f: string): number {
  const out = Math.trunc(
    (f
      .split("-")
      .map((x) => x.trim())
      .map((x) => parseInt(x))
  ...
  return routes
    ...
    .map((route) => {
      ...
      let service = services.filter(
        (service) =>
          service.ServiceNo == route.ServiceNo &&
          service.Direction == route.Direction
      )[0];
      ...
      for (const day of days) {
        for (const period of Object.keys(periods)) {
          if (route[`${day}_FirstBus` as keyof DataMallBusRoute] === "-") {
            continue;
          }

          if (service[`${period}_Freq` as keyof DataMallBusService] === "-") {
            continue;
          }

          if (period === "AM_Peak") {
            // For services that start before am peak period
            const firstBus = parseInt(
              route[`${day}_FirstBus` as keyof typeof route] as string
            );

            if (firstBus < 630 && service.AM_Offpeak_Freq != "-") {
              const headway = getMeanFrequency(service.AM_Offpeak_Freq);

              const firstBusStr = route[
                `${day}_FirstBus` as keyof typeof route
              ] as string;
              out.push({
                ...route,
                trip_id: `${route.ServiceNo}_${route.Direction}_${day}`,
                service_id: day,
                start_time: `${firstBusStr.slice(0, 2)}:${firstBusStr.slice(
                  2,
                  4
                )}:00`,
                end_time: "06:29:00",
          ...
          const headway = getMeanFrequency(
            service[`${period}_Freq` as keyof DataMallBusService] as string
          );

          if (period === "PM_Offpeak") {
            const lastBus = route[
              `${day}_LastBus` as keyof typeof route
            ] as string;
            ...
            out.push({
              ...
              trip_id: `${route.ServiceNo}_${route.Direction}_${day}`,
              service_id: day,
              start_time: periods[period as keyof typeof periods][0],
            ...
            out.push({
              ...
              service_id: day,
              start_time: periods[period as keyof typeof periods][0],
              end_time: periods[period as keyof typeof periods][1],
              headway: headway,
            });
          }
        }
